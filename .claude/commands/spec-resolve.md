...
3. **사용자 결정 직후 즉시 Edit 적용.** AskUserQuestion으로 받은 결정을 그대로 본문에 반영하고 별도 "이 Edit을 적용해도 될까요?" 식의 명시 승인 라운드를 추가하지 않는다. 결정 자체가 곧 승인이다. 단, Edit 적용 직전에 *어디에 어떤 문장을 넣을지*는 짧게 보고한 뒤 바로 Edit한다(보고 ≠ 승인 요청).
...
각 결정 항목에 대해 **어디에 어떻게 반영할지**를 본인이 선택해 즉시 Edit한다. 가능하면 단계 2의 질문 라운드 안에 "어디에 반영할지" 옵션까지 묶어 한 번에 결정을 받는다(예: "Behavior에 한 줄 추가" / "Out of Scope에 한 줄 추가" / "Spec 본문 변경 없이 Open Question만 닫기"). 결정이 끝났으면 별도 승인 라운드 없이 바로 Edit한다.
...
Edit 직전에 어떤 문장을 어디에 넣는지 1~3줄로 짧게 보고만 하고 바로 Edit한다. "이대로 적용해도 될까요?" 같은 명시 승인 질문을 추가하지 않는다.

### 4. Edit 가드레일

- 본문 추가/수정은 **최소 범위 Edit**로. 기존 문장 톤과 맞춘다.
- 닫힌 Open Question 항목은 해당 줄을 삭제하거나 구체화된 형태로 교체한다. 모든 항목이 닫혀 섹션이 비면 parent `_index.md`와 동일한 `_현재 열린 질문 없음._` 표기로 교체한다.
