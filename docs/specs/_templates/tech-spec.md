...
<본 sub-spec의 대상이 기존 자산(예: 같은 카테고리 악기/sub-spec)과 동일 카테고리이면 차이를 표로 박제한다. 동급 자산이 없으면 `_해당 없음 — 신규 카테고리_` 한 줄. spec-design-extractor가 후보를 자동 탐색하지만, 인터뷰 라운드에서 검증·보강한다.>
