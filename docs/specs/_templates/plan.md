...
- 최소 1개 항목, 최대 ~6개 권장.
- 각 항목 끝에 출처를 명시한다:
  - `MCP <tool> <action> (YYYY-MM-DD)` — `/plan-new` step 1.5에서 read-only Unity MCP 도구를 메인 세션이 직접 호출해 검증한 경우. 예: `MCP manage_prefabs get_hierarchy (2026-05-03)`, `MCP manage_components get (2026-05-03)`. (권장)
...
- <검증 항목 1 — 예: "VR Player.prefab의 controller-tracked transform은 `Camera Offset/Hands/Left/LeftControllerHandRoot`(TrackedPoseDriver 부착) — `Left` 자체는 정적 컨테이너"> — `MCP manage_prefabs get_hierarchy (YYYY-MM-DD)`
- <검증 항목 2> — `MCP manage_components get (YYYY-MM-DD)`
