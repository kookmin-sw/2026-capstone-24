...
- 2026-05-07: 검증 실패에서 파생된 후속 plan `2026-05-07-linksky0311-session-panel-instance-volume-verification.md` 추가. 완료 후 본 plan의 `[manual-hard]` "dummy provider Current를 Piano로 set → 인스턴스 슬라이더 1개 추가 노출 (라벨 "Piano"). 슬라이더 변경 시 Pian" 항목 재검증 필요.
- 2026-05-07: 검증 실패에서 파생된 후속 plan `2026-05-07-linksky0311-session-panel-instance-volume-verification.md` 추가. 완료 후 본 plan의 `[manual-hard]` "dummy provider Current를 DrumKit으로 swap → 인스턴스 슬라이더의 라벨/value가 DrumKit 기준으로 갱신. 슬라이더 변경 시 D" 항목 재검증 필요.

## Handoff

### 씬 변경사항 (SampleScene.unity)
- EventSystem: InputSystemUIInputModule (m_EnableXRInput: 1) 추가
- SessionPanel Canvas: TrackedDeviceGraphicRaycaster + GraphicRaycaster 공존
- VR Player 컨트롤러 레이:
  - Right_NearFarInteractor.prefab → RightControllerHandRoot/Near-Far Interactor
  - Left_NearFarInteractor.prefab → LeftControllerHandRoot/Near-Far Interactor
  - trigger 클릭 + pinch(hand UI Press) 바인딩 내장

### Audio 수정
- SessionMixer.mixer: `MasterVolume_dB` exposed parameter 이름 설정
  (이전에는 빈 문자열로 SetFloat("MasterVolume_dB") 호출이 무시됨)

### 미완료 (deferred)
- 인스턴스 슬라이더(AC #12/#13): VolumeSectionController 코드는 구현됨,
  활성 악기 선택 UI(DummyActiveInstrumentProvider 실 구현체) 없어 VR 검증 불가
- Hand tracking UI 레이: XRHandDevice TrackedPoseDriver 충돌로 롤백, 추후 개발
