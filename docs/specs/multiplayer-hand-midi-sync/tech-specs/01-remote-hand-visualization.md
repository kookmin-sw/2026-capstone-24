...
- **매 틱 (inputAuthority 클라이언트):** PlayHand `R_Wrist` / `L_Wrist` 본 global pose + 손가락 본 10개 localRotation ([decision 04](../decisions/04-hand-pose-payload-reduction.md) supersede) → `LocalHandPoseSource` → `NetworkedHandPose` 의 [Networked] 필드 write.
...
- PlayHand prefab 의 본 계층은 손당 총 26개 본 (wrist 1 + Index/Middle/Ring/Little 각 5단 = 20 + Thumb 4단 = 4 + Palm 1). 본 이름 prefix 는 우측 `R_`, 좌측 `L_`. 출처: `Read Assets/Hands/Prefabs/Play/RightPlayHand.prefab + LeftPlayHand.prefab (2026-05-19)`.
  _네트워크 동기화 대상은 wrist + 10본 (Proximal + Intermediate × 4 fingers + Thumb Proximal/Distal) — [decision 04](../decisions/04-hand-pose-payload-reduction.md) 박제. 나머지 본 (Metacarpal/Distal/Tip/Palm) 은 RemoteHandRenderer 의 bind pose default 로 정지._
...
- [x] 손 pose 송신을 [Networked] 본 단위 quaternion 직접 송신 vs 압축 pose snapshot 으로 할 것인지. → [decisions/01-hand-pose-network-encoding.md](../decisions/01-hand-pose-network-encoding.md) (2026-05-27 decision 04 가 Capacity / throttle / channel 정책으로 supersede — [decisions/04-hand-pose-payload-reduction.md](../decisions/04-hand-pose-payload-reduction.md))
