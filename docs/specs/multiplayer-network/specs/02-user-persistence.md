...
서버에서 인증된 유저 정보를 DB에 저장하고 조회하는 기능을 제공한다. 최초 로그인 시 유저 레코드를 생성하고, 재접속 시 기존 레코드를 조회해 같은 유저로 연결한다. DB 내부 조인과 FK는 `users.id`(내부 PK)를 사용하고, 외부 시스템과 클라이언트가 참조하는 영구 플레이어 식별자는 `users.player_id`로 분리한다. `metaAccountId`는 로그인 제공자 식별자, `nickname`은 표시용 값으로만 취급한다.
