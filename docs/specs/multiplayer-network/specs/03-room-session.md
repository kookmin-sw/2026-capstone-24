...
Photon Fusion을 통해 룸을 생성하거나 기존 룸에 입장·퇴장할 수 있는 세션 관리 기능을 제공한다. 여러 유저가 같은 룸에 동시 접속해 공유 공간을 형성한다. 룸 권위는 전용 서버 인스턴스가 가지며, 클라이언트는 서버에 연결해 룸을 생성·입장한다. 모든 룸은 공개되어 룸 목록에서 검색·입장할 수 있다.
