...
현재 인메모리 기반인 유저 저장소를 영속 DB 기반으로 전환해, 서버 재시작 이후에도 같은 Meta 계정이 동일한 유저 레코드로 유지되도록 만든다.
...
- 현재 유저 모델은 [UserProfile.java](/D:/2026-capstone-24/backend/src/main/java/com/murang/user/domain/UserProfile.java)의 `userId`, `metaAccountId`, `nickname`, `createdAt`, `lastLoginAt` 다섯 필드다.
- 현재 `refresh` 흐름은 저장된 `metaAccountId`로 유저를 다시 조회해 JWT를 재발급하는 구조라, `findByMetaAccountId`의 안정성이 핵심이다.
...
2. `UserProfile` record와 별도로 JPA 엔티티를 만들고, `metaAccountId`/`nickname` 유니크 제약과 생성/수정 시각 컬럼을 정의한다.
3. `UserRepository`를 추가하고 `findByMetaAccountId`, `findByNicknameIgnoreCase` 같은 조회 경로를 명시한다.
4. `InMemoryUserRegistry`를 대체할 `PersistentUserRegistry`를 구현하고, 기존 `registerOrUpdate`/`findByMetaAccountId` 계약을 유지한다.
